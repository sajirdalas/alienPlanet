...
function handleHide() {
  ...
  //update the state of the button in the current tab
...
tabs.on('open', function(tab){
  ...
  tabMap[tab.id].viewState = 0;
  tabMap[tab.id].redditPosts = reddit.getEmpyRedditPostsObject("");
  updateIcon(tabMap[tab.id].redditPosts, button, tab);
});
...
function changePanel(tab){
  ...
  updateIcon(reddit.getEmpyRedditPostsObject(""), button, tab);
  tabMap[tab.id].lastURL = tab.url;
  var currentTab = tab;
  var currentPanel = tabMap[tab.id];
  currentPanel.port.emit("loadingScreen",true);
...
function createNewPanel(width, height){
  ...
  panel.port.on("goBack",function(){
    var tab = tabs.activeTab;
    var currentPanel = tabMap[tab.id];
    var badgeText = currentPanel.redditPosts.posts.length;

    panelHelper.panelListMode(currentPanel.redditPosts, currentPanel, button, self, function(newPanel){
        tabMap[tab.id] = newPanel;
        tabMap[tab.id].viewState = 1;
        updateIcon(newPanel.redditPosts, button, tab);
        button.state('window', {
          checked: true,
        });
        newPanel.show({
  ...
  //This guy is experimental as fuck

  panel.port.on("commentMode", function(url){
    reddit.getRedditComments(url, function(comments){
      panel.port.emit("receiveComments", comments)
    });
  });
...
function updateIcon(redditResult, button, tab) {
